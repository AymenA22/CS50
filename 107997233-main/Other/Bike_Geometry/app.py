...
def buy():
    ...
    # If method is GET return to buy.html else POST
...
def history():
    ...
    return render_template("history.html", transactions_database = transactions_db)
...
def add():
    ...
    # else method == POST
        ...
        additional_funds = request.form.get("additional_funds")
        ...
        update_cash = float(user_cash) + float(additional_funds)
        ...
        # Add a new row to the transaction database with a timestamp
        #transaction_date = datetime.datetime.now()
        #db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date) VALUES (?, ?, ?, ?, ?)", user_id, "Cash Paid In", (1) , additional_funds, transaction_date)
