...
def complete_reminder():
    ...
    if reminder:
        ...
        db.execute("INSERT INTO Reminders_Archive (user_id, Content, Input_Date, Priority) VALUES (?, ?, ?, ?)", user_id, event, date_string, priority)
        db.execute("DELETE FROM Reminders WHERE id=?", (id,))
...
def complete_ToDo():
    ...
    if todo:
        ...
        db.execute("INSERT INTO ToDo_Archive (user_id, Content, Input_Date, Completion_Date) VALUES (?, ?, ?, ?)", user_id, event, date_string, priority)
        db.execute("DELETE FROM ToDo WHERE id=?", (id,))
...
def add():
    ...
    if request.method == "GET":
        ...
        if note:
            db.execute("INSERT INTO Notes (user_id, Content, Date, Priority) VALUES (?, ?, ?, ?)", user_id, event, date_string, priority)
            flash("Note Added")
            return redirect("Notes")

        if reminder:
            db.execute("INSERT INTO Reminders (user_id, Content, Input_Date, priority) VALUES (?, ?, ?, ?)", user_id, event, date_string, priority)
            flash("Reminder Added")
            return redirect("Reminders")

        if todo:
            db.execute("INSERT INTO ToDo (user_id, Content, Completion_Date) VALUES (?, ?, ?)", user_id, event, date_string)
            flash("Task Added")
            return redirect("To-Do")
